...
def startScreen():
    ...
    print("\n")
    print(RED+"__________         __    __  .__         "+PURPLE+"       .__    .__        ")
    print(RED+"\______   \_____ _/  |__/  |_|  |   ____ "+PURPLE+"  _____|  |__ |__|_____    ")
    print(RED+" |    |  _/\__  \\   __\   __\  | _/ __ \ "+PURPLE+" /  ___/  |  \|  \____  \  ")
    print(RED+" |    |   \ / __ \|  |  |  | |  |_\  ___/ "+PURPLE+"\___ \|   Y  \  |  |_> >  ")
    print(RED+" |______  /(____  /__|  |__| |____/\___  >"+PURPLE+"____  >___|  /__|   __/   ")
    print(RED+"        \/      \/                     \/ "+PURPLE+"    \/     \/   |__|      "+ENDC)
    ...
    print("                           |   "+OKGREEN+"x    x  x"+YELLOW+"   |")
    print("           \\----|------------------------------------------/")
    print("	    \\                                             /")
    print(OKBLUE+"   /\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\  ")
    print("\n")
    print("                           Credits: M & M                            "+ENDC)
    print("\n")
    ...
    #menü
    while 1==1:
        command = input("Type '"+OKGREEN+"s"+ENDC+"' to start single screen game, '"+OKBLUE+"m"+ENDC+"' for multi screen game or '"+RED+"c"+ENDC+"' to close. ")
        if command == "s":
            placeships()
            break
        elif command == "m":
...
def turn(map, mapDraw, player, playeronthismachine = 2):
    ...
    whereToShoot = input("Choose where would you like to shoot! ")
    ...
    try:
        ...
        input("Invalid format! Please give the coordinates like this: A 5 [press enter to continue]")
        changePlayer = False
    
    #check coordinates for index error
    if changePlayer == True and not (x >= 0 and x < 10 and y >= 0 and y < 10):
        changePlayer = False
        input("Your coordinates are out of range. The table is 10 * 10 big! [press enter to continue]")
    ...
    if changePlayer == True:
        ...
        if mapDraw[player-1][x][y] == "O":
            ...
            input("You shot here already before. Choose other coordinates! [press enter to continue]")
            changePlayer = False
        ...
        #a játékosok külön képernyőn tolják
            ...
            while True:
                ...
                if answer[0] == "[error]" or int(answer[0]) == int(lastEnemyShotId):
                    print("Waiting for enemy to shot. Maybe he is high?")
                    time.sleep(5)
                else:
...
def winSituation(player):
    ...
    if player == 1:
        print()
        print(YELLOW+"   __________.__                               ____ "+RED+"                     ._.")
        print(YELLOW+"   \______   \  | _____  ___.__. ___________  /_   |"+RED+" __  _  ______   ____| |")
        print(YELLOW+"    |     ___/  | \__  \<   |  |/ __ \_  __ \  |   | "+RED+"\ \/ \/ /  _ \ /    \ |")
        print(YELLOW+"    |    |   |  |__/ __ \\___  \  ___/|  |  \/  |   |"+RED+"  \     (  <_> )   |  \|")
        print(YELLOW+"    |____|   |____(____  / ____|\___  >__|     |___| "+RED+"  \/\_/ \____/|___|  /_")
        print(YELLOW+"                       \/\/         \/               "+RED+"                  \/\/"+ENDC)
        print()
    elif player == 2:
        print()
        print(OKGREEN+" __________.__                              ________  "+RED+"                     ._.")
        print(OKGREEN+" \______   \  | _____  ___.__. ___________  \_____  \ "+RED+" __  _  ______   ____| |")
        print(OKGREEN+"  |     ___/  | \__  \<   |  |/ __ \_  __ \  /  ____/ "+RED+" \ \/ \/ /  _ \ /    \ |")
        print(OKGREEN+"  |    |   |  |__/ __ \\___  \  ___/|  |  \/ /       \ "+RED+"  \     (  <_> )   |  \|")
        print(OKGREEN+"  |____|   |____(____  / ____|\___  >__|    \_______ \  "+RED+" \/\_/ \____/|___|  /_")
        print(OKGREEN+"                     \/\/         \/                \/  "+RED+"                 \/\/"+ENDC)
        print()
...
def drawMap(map, mapID):
    ...
    print(OKBLUE+"".join(col_names)+ENDC, end="\n")
    for i in range(0,10):
        print(OKBLUE+str(row_names[i])+ENDC+" " + " ".join(map[mapID][i]))
...
def ship(length, shipname, player):
    ...
    while True:
        ...
        print()
        print("To give orientation to your ships use H for horizontal, V for verical.")
        carrier = input("Give coordinates for "+RED+shipname+ENDC+" [e.g. H A 1]: ")
            ...
            if len(shipCoo) == 3:
                if shipCoo[0] == "H" or shipCoo[0] == "V":
                    if shipCoo[1] in row_names:
                        ...
                        carrier = input("Give orientation and coordinates for "+RED+shipname+ENDC+": ")
                        shipCoo = carrier.split(" ")
                else:
                    carrier = input("Give orientation and coordinates for "+RED+shipname+ENDC+": ")
                    shipCoo = carrier.split(" ")
            ...
            if shipCoo[0] == "H":
                for i in range(length):
                    if map[player][int(shipCoo[1])-1][int(shipCoo[2])+ i - 1].isdigit():
                        x = 0
                print ("Ooops, you have already placed a ship there!")
...
def apiComm(action, player=False, option=False, value = False):
    ...
    #insert data to the database
        ...
        #sending request
        urllib.request.urlopen('http://battleshipio.standapp.hu/set/'+str(player)+'/'+str(option)+'/'+str(value))

    #emptying out the database
    elif action == "truncate":
        
        #sending request
        urllib.request.urlopen('http://battleshipio.standapp.hu/truncate')
